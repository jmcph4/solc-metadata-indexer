...
const IPFS_DIGEST_LEN: usize = 43;
const SWARM_DIGEST_LEN: usize = 43; /* TODO(jmcph4): placeholder */
...
fn grab_swarm_digest(data: &[u8]) -> Option<Vec<u8>> {
    if data.len() >= SWARM_DIGEST_LEN {
        Some(data[data.len() - IPFS_DIGEST_LEN..].to_vec())
    } else {
        None
    }
}
